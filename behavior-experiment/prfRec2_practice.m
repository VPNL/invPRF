function [params,expt,block,perf,eyeInit] = prfRec2_practice(initials,face)
% version 2: 2AFC trials with paired faces presentation & phase-scrambled
% mask.
%clear all; close all;
%initials = 'testJJ';
...
expt.feedback = 1;  % giving feedback on the same/different task
...
params.blockTrials = 4;                 % number of trials in each block (/2 orientations). must be /4 for equal numbers of hits and misses
...
% approximate
fprintf('Practice will take approx. %.2f mins.\n',...
...
for n = 1:length(block)
    ...
    if n>1
        WaitSecs(1);
        DrawFormattedText(win,['You have completed ' num2str(n-1) ' of ' num2str(params.locReps * numLocs) ' practice blocks.\n' ...
            'Take a break if you need it. \n\nPress Space to Advance, and don''t forget to FIXATE throughout each trial.']...
            ,'center', 'center',[0 0 0]);
        Screen(win, 'Flip', 0);
        fprintf('Subject has completed practice block %d of %d...\n',n-1,params.locReps * numLocs);
        KbQueueTriggerWait(external,'Space');
    end
    for t = 1:params.blockTrials
        %***!!!***!!!***!!!***!!!***!!!***!!!%
        ...
        %***!!!***!!!***!!!***!!!***!!!***!!!%
        if expt.eyetrack eyelinkEndTrial('PRACTRIALEND'); end
...
DrawFormattedText(win,['You have completed the practice!']...
    ,'center', 'center',[0 0 0]);
WaitSecs(1);
...
if expt.eyetrack
    eyelinkClose;
    % give participant feedback about fixation
    %%% eyetracking feedback - currently selects the second half of each
    %%% block
    
    tr = 1;
    for n = 1:length(block)
        for t = round(length(block(n).trial)/2):length(block(n).trial)
            gaze(tr).x = block(n).trial(t).gazeX; gaze(tr).y = block(n).trial(t).gazeY;
            gaze(tr).SDs = block(n).trial(t).SDs; % drift correction deviation
            if expt.driftCorr
                gaze(t).x = gaze(n).x+block(n).trial(t).corrX;
                gaze(t).y = gaze(n).y+block(n).trial(t).corrY;
            end
            tr = tr+1;
        end
    end
...
for t = 1:length(gaze)
    gaze(t).xSD = nanstd([gaze(t).x])/expt.ppd;
    gaze(t).ySD = nanstd([gaze(t).y])/expt.ppd;
...
medXdev = nanmedian([gaze(:).xSD]); medYdev = nanmedian([gaze(:).ySD]);
...
fprintf('median SDs during driftCorr = %.2fdva\n', nanmedian([gaze(:).SDs]./expt.ppd))

DrawFormattedText(win,[perfText '\nPress Space to proceed.'], 'center',yc *.25 ,params.textColor);
Screen(win, 'Flip', 0);fixIm=Screen('GetImage', win); imwrite(fixIm,['pracFix/' initials '_practiceFix.png'],'png');

KbQueueTriggerWait(external,'Space');
end
%%% behavioral feedback
feedbackCond = 5; % make sure this is upright-center

if perf(feedbackCond).percCorrect > .8 perfText = 'Great!';
elseif perf(feedbackCond).percCorrect > .5 perfText = 'Good.';
else perfText = 'Fair. Recommend more practice trials.'; end
fprintf('%s percent correct: = %.2f%%,  %s\n',perf(feedbackCond).condName,...
    perf(feedbackCond).percCorrect, perfText);

perfText = ['You got ' (num2str(perf(feedbackCond).percCorrect)) '% of the trials correct '...
    'when the faces were upright\nand centrally presented. This performance is ' perfText];
DrawFormattedText(win,[perfText '\nPress Space to proceed.'], 'center','center',params.textColor);
Screen(win, 'Flip', 0);
KbQueueTriggerWait(external,'Space');
